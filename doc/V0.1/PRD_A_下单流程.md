...
订单支付页是用户完成支付的关键页面，展示待支付金额、可用积分余额和四种支付方式供选择。用户从确认订单页点击"立即支付"进入此页面，选择支付方式并确认支付后跳转支付结果页。页面支持微信支付、支付宝、积分全额兑换、积分+现金组合支付四种方式，选择组合支付时展开抵扣明细（含积分批次消耗明细）；选择全额兑换且余额不足时，显示需补充的现金金额并建议使用组合支付。积分消耗按过期时间升序排列，优先消耗最先过期的积分批次。
...
| 积分余额 | 展示星形图标 + "可用积分：1,280"（橙色数字）+ "1积分=1元"换算提示 |
| 支付方式列表 | 标题"选择支付方式" + 4种支付方式卡片，每种包含图标、名称、描述和单选圆圈 |
| 组合支付明细 | 仅在选择"积分+现金组合支付"时展开，显示积分抵扣金额、积分消耗批次明细（按过期时间升序）、现金支付金额和合计 |
| 余额不足提示 | 仅在选择"积分全额兑换"且余额不足时显示，橙色背景提示卡片，显示缺少积分数和需补充金额，建议使用组合支付 |
...
```mermaid
flowchart TD
    A[进入订单支付页] --> B[查看支付金额与积分余额]
    B --> C{选择支付方式}
    C -->|微信支付| D[选中微信支付]
    C -->|支付宝支付| E[选中支付宝支付]
    C -->|积分全额兑换| F[选中全额兑换]
    C -->|积分+现金组合| G[选中组合支付 + 展开抵扣明细]
    D --> H[点击确认支付]
    E --> H
    F --> H
    G --> H
...
支付方式为单选互斥：点击任一方式后，其他方式的选中状态自动取消，选中方式的单选圆圈变为橙色填充。选择"积分+现金组合支付"时，下方自动展开橙色背景的抵扣明细卡片，显示积分抵扣金额、现金补齐金额和合计；切换其他支付方式时该卡片自动收起。
...
| 积分余额 | points_balance | 文本显示 | - | Number | - | - | 橙色加粗数字，显示可用积分数量 | ≥0 | 后端接口 | - |
| 换算提示 | points_rate | 文本显示 | - | String | - | "1积分=1元" | 灰色小字，说明兑换比例 | - | 系统配置 | - |
| 微信支付 | method_wechat | 单选项 | - | - | - | 选中 | 绿色图标+名称+描述"推荐使用，安全便捷"，默认选中 | - | 静态配置 | - |
| 支付宝支付 | method_alipay | 单选项 | - | - | - | 未选中 | 蓝色图标+名称+描述"H5场景可用" | - | 静态配置 | - |
| 积分全额兑换 | method_beans | 单选项 | - | - | - | 未选中 | 橙色图标+名称+描述"需消耗X积分，当前余额不足，需补充¥X.XX"，余额不足时橙色高亮提示补充金额 | - | 后端计算 | - |
| 积分+现金组合 | method_combo | 单选项 | - | - | - | 未选中 | 渐变图标+名称+描述"使用全部积分抵扣，剩余现金补齐"，选中时展开明细 | - | 静态配置 | - |
| 组合抵扣明细 | combo_detail | 折叠面板 | - | - | - | 隐藏 | 仅combo选中时显示，橙色背景卡片，列出积分抵扣金额、积分消耗批次明细（按过期时间升序，每行显示积分数+到期日期+抵扣金额）、现金支付金额、合计 | 显示/隐藏 | 系统计算 | - |
...
| RULE-PAY-002 | 积分全额兑换方式展示所需豆数，余额不足时描述文案橙色高亮提示 |
| RULE-PAY-003 | 组合支付明细根据选中状态动态展开/收起，切换其他方式时自动收起 |
| RULE-PAY-004 | 底部支付按钮的金额文案与页面顶部支付金额保持一致 |
| RULE-PAY-005 | 选择"积分全额兑换"时，若余额不足，描述文案显示需补充金额（补充金额=商品价格-现有积分价值），同时下方显示橙色提示卡片建议使用组合支付 |
| RULE-PAY-006 | 选择"积分+现金组合支付"时，展开的抵扣明细包含"积分消耗明细"子区域，逐行列出按过期时间升序排列的积分批次（每批显示：积分数+到期日期+抵扣金额），现金支付金额=商品价格-全部积分抵扣金额 |
...
| 订单信息卡片 | 白色圆角卡片，逐行列出订单编号、支付方式、积分抵扣、现金支付、实付金额 |
...
| 支付方式 | order_pay_method | 文本显示 | 是 | String | - | - | 如"积分+现金组合" | - | 订单支付选择 | - |
| 积分抵扣 | order_beans_deduct | 文本显示 | - | String | - | - | 格式"1,280豆 (¥1,280.00)" | - | 系统计算 | - |
