...
| RULE-PD-010 | 点击"加入购物车"时校验库存上限：同一SKU已在购物车中的数量 + 本次加购数量 > 库存时，Toast提示"库存仅剩X件，购物车已有Y件"，不执行加购 |
...
| 规则编号 | 规则描述 |
|----------|----------|
| RULE-CART-001 | 库存扣减漏斗策略：加购时不扣真实库存（彻底无货拦截"已售罄"，库存紧张静默加购，同一SKU累计加购数量超过库存时提示"库存仅剩X件，购物车已有Y件"不执行加购）；结算时校验库存（红字提示"部分商品无库存"，刷新购物车页面数据，不区分部分或全部缺货）；支付时真实扣减 |
...
| RULE-CART-006 | Tab栏购物车角标显示有效商品SKU的数量（SKU维度去重，排除已下架/已售罄/无库存），大于99显示99+ |
...
| RULE-ORDER-017 | 提交订单时校验商品上下架状态，已下架商品提示"部分商品已下架"，刷新确认订单页数据 |
| RULE-ORDER-018 | 提交订单时校验商品库存，库存不足商品提示"部分商品库存不足"，刷新确认订单页数据 |
...
| 操作按钮 | 按状态显示：待付款(取消订单+去支付)、待发货(查看详情)、待收货(查看物流+确认收货)、已完成(再次购买+评价)、已取消/已关闭(查看详情) |
...
| RULE-ODLIST-002 | 未支付订单取消时直接作废，无退款流程 |
...
- 取消订单 → 取消订单页（order_cancel.html?status=pending）
...
| 底部操作栏 | 按状态显示：待付款(取消订单)、待发货(无按钮)、待收货(查看物流+确认收货)、已完成(无按钮) |
...
取消订单页展示订单摘要和取消原因选择。用户选择取消原因后点击确认，通过二次确认弹窗确认取消操作。仅支持待付款订单取消。

#### 5.5.2 页面结构

页面从上到下分为：订单信息卡、取消原因选择区、温馨提示和底部操作栏。另有确认取消弹窗。
...
**入口**：
- 订单列表点击"取消订单"（待付款：?status=pending）
- 订单详情点击"取消订单"（待付款）
