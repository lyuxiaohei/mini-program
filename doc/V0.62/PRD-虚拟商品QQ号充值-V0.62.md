...
- 卡片脱敏：手机号 `138****1111`；QQ号（≥5 位）保留首尾各 1 位 `1****9`。
...
确认账号后，订单页"充值账号"行展示带类型前缀的脱敏账号：`手机号：138****1111` / `QQ号：1****9`（卡密链路保持 V0.6 无前缀脱敏展示）。

---

## 五、直充订单详情页（order_detail_virtual_direct.html）

充值信息卡片「充值账号」行由原明文账号改为**类型前缀 + 账号**：

- 读取 URL `?accountType=`，默认 `phone`。
- 展示：`手机号：13800001111` 或 `QQ号：123456789`（账号明文，不脱敏）。
...
| AC-V062-008 | 详情页账号行展示类型 | 进入直充订单详情页（?accountType=qq） | 查看充值账号行 | 显示"QQ号：123456789" |
